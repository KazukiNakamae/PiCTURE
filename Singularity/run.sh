...
if [[ ! -f 1_trim_galore/${input_name}_F_val_1.fq ]]; then
    ...
    singularity exec --bind `pwd`:/DATA -W /DATA ../trim_galore.sif \
        trim_galore \
        -j 14 --fastqc --trim1 \
...
if [[ ! -f 2_star/star_${input_name}_SJ.out.tab ]]; then
    ...
    singularity exec --bind `pwd`:/DATA -W /DATA ../star.sif \
        STAR \
...
if [[ ! -f 4_bam_preparation/${input_name}.complete.bam ]]; then
    ...
    bash ./bam_preparation_v2.sh resources-broad-hg38-v0-Homo_sapiens_assembly38.fasta \
